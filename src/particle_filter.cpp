...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> landmarks, std::vector<LandmarkObs> &observations) {
    ...
    for(int a = 0; a < observations.size(); a++){
        int index = 0;
        double min_distance = distance(observations[a], landmarks[0]);
        for(int b = 1; b < landmarks.size(); b++){
            double curr_distance = distance(observations[a], landmarks[b]);
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], std::vector<LandmarkObs> observations, Map map_landmarks) {
    ...
    double pix2xsigxxsigy = 2 * M_PI * std_landmark[0] * std_landmark[1];
    ...
    for(int a = 0; a < num_particles; a++){
        ...
        for(int b = 0; b < transformedObservations.size(); b++){
            ...
            // if (weight > 0)
            particles[a].weight *= weight; // TODO see if this is necessary
        }
